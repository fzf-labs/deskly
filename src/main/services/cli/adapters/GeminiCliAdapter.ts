...
export class GeminiCliAdapter implements CliAdapter {
  ...
  buildCommandSpec(options: CliStartOptions): ProcessCommandSpec {
    ...
    pushFlagWithValue(args, '--approval-mode', (toolConfig as Record<string, unknown>).approval_mode)
    pushRepeatableFlag(args, '--policy', (toolConfig as Record<string, unknown>).policy)
    pushFlag(args, '--acp', asBoolean((toolConfig as Record<string, unknown>).acp))
    pushRepeatableFlag(args, '--allowed-mcp-server-names', (toolConfig as Record<string, unknown>).allowed_mcp_server_names)
    pushRepeatableFlag(args, '--allowed-tools', (toolConfig as Record<string, unknown>).allowed_tools)
    pushRepeatableFlag(args, '--extensions', (toolConfig as Record<string, unknown>).extensions)
    pushFlagWithValue(args, '--resume', resumeId)
    pushRepeatableFlag(args, '--include-directories', (toolConfig as Record<string, unknown>).include_directories)
    pushFlagWithValue(args, '--output-format', (toolConfig as Record<string, unknown>).output_format)
    pushFlag(args, '--raw-output', asBoolean((toolConfig as Record<string, unknown>).raw_output))
    pushFlag(args, '--accept-raw-output-risk', asBoolean((toolConfig as Record<string, unknown>).accept_raw_output_risk))
        ...
        try {
          ...
          if (maybePromise && typeof (maybePromise as Promise<void>).then === 'function') {
            void (maybePromise as Promise<void>).catch((error: unknown) => {
              console.warn('[GeminiCliAdapter] Failed to persist resume id:', error)
            })
          }
        } catch (error) {
          console.warn('[GeminiCliAdapter] Failed to persist resume id:', error)
