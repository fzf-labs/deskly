...
import type { SkillInfo } from '@features/settings'
import { Button } from '@/components/ui/button'
import { EmptyStatePanel, PageBody, PageFrame, PageHeader } from '@/components/shared/page-shell'
import { useProjects } from '@features/projects'
import { useLanguage } from '@/providers/language-provider'
import { getSettings } from '@/data/settings'
import { FolderOpen, Loader2, MoreHorizontal, RefreshCw, Trash2, X } from 'lucide-react'
import {
...
export function SkillsPage() {
  ...
  const loadProjectSkills = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const cliTools = (await window.api?.cliTools?.getAll?.())?.filter(Boolean) ?? []
      const fallbackTools = cliTools.length ? cliTools : [{ id: 'codex', displayName: 'Codex' }]
  ...
  const handleRefreshGroup = async (group: CliSkillGroup, scope: 'global' | 'project') => {
    try {
      if (scope === 'global') {
        const skills = await loadSkillsFromDirectory(group.globalPath, 'global')
        setCliGroups((prev) =>
          prev.map((item) => (item.id === group.id ? { ...item, globalSkills: skills } : item))
        )
        return
      }
      const skills = await loadSkillsFromDirectory(group.projectPath, 'project')
      setCliGroups((prev) =>
        prev.map((item) => (item.id === group.id ? { ...item, projectSkills: skills } : item))
      )
    } catch (err) {
  ...
  const copyDirectoryRecursive = useCallback(async (sourcePath: string, targetPath: string) => {
  ...
  const handleImportFromGlobal = useCallback(async () => {
    ...
    setImporting(true)
    try {
      const skillsToImport = globalSkills.filter((skill) => selectedGlobalSkills.includes(skill.id))
      const targets = cliTargets.filter((target) => selectedCliTargets.includes(target.id))
  ...
  return (
    <PageFrame>
      <PageHeader
        title="Skill"
        subtitle={currentProject ? currentProject.name : '未选择项目'}
        actions={
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={loadProjectSkills} disabled={!currentProject?.id}>
              刷新
            </Button>
            <Button onClick={openGlobalImport} disabled={!currentProject?.id}>
              从全局导入
            </Button>
          </div>
        }
      ...
      <PageBody>
        {!currentProject ? (
          <EmptyStatePanel
            title="未选择项目"
            description="请选择一个项目以查看或管理项目级 Skills。"
          />
        ) : (
                ...
                cliGroups.map((group) => (
                  ...
                  >
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                      ...
                      <div className="border-border rounded-lg border p-3">
                        <div className="flex items-center justify-between gap-2">
                          <div className="text-foreground text-sm font-medium">Skill</div>
                          <div className="flex items-center gap-2">
                            <button
      ...
      {deleteDialogSkill && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black/50" onClick={() => setDeleteDialogSkill(null)} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-background border-border relative z-10 w-[520px] rounded-xl border p-6 shadow-lg">
            ...
            <button
              ...
              disabled={
                importing || selectedGlobalSkills.length === 0 || selectedCliTargets.length === 0
              }
