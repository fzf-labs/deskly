...
import type { DocxParagraph, PreviewComponentProps } from '../model/types'
  ...
  useEffect(() => {
    async function loadDocx() {
      ...
      try {
        ...
        console.log('[DOCX Preview] Parsed', parsedParagraphs.length, 'paragraphs')
  ...
  if (loading) {
    return (
      <div className="bg-muted/20 flex h-full flex-col items-center justify-center p-8">
        ...
        <p className="text-muted-foreground mt-4 text-sm">Loading document...</p>
      </div>
    )
  }
        ...
        <div className="mx-auto max-w-3xl">
          {paragraphs.map((para, idx) => {
            if (para.isHeading || para.style?.toLowerCase().includes('title')) {
              const level =
                para.headingLevel || (para.style?.toLowerCase().includes('title') ? 1 : 2)
