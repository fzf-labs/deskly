...
type CursorProcessKind = 'command' | 'file_read' | 'file_edit' | 'tool'

type CursorTimelineItem =
...
function processKindFromEntry(entry: NormalizedEntry): CursorProcessKind {
  if (entry.type === 'command_run') return 'command'
...
function processTitle(kind: CursorProcessKind, entry: NormalizedEntry): string {
...
export function buildCursorAgentTimelineItems(entries: NormalizedEntry[]): CursorTimelineItem[] {
  const items: CursorTimelineItem[] = []
  ...
  const flushAssistantBuffer = () => {
    ...
    if (content.trim()) {
      items.push({
        id: `cursor-answer-${assistantBuffer[0]?.id ?? items.length}`,
        type: 'answer_block',
  ...
  entries.forEach((entry, index) => {
    if (entry.type === 'user_message') {
      ...
      items.push({ id: `cursor-user-${entry.id}`, type: 'user_bubble', entry })
      return
    }
    ...
    if (entry.type === 'error') {
      items.push({ id: `cursor-error-${entry.id}`, type: 'error_block', entry })
      return
    }

    if (entry.type === 'system_message') {
      items.push({ id: `cursor-status-${entry.id}`, type: 'status_block', entry })
      return
    }
    ...
    items.push({ id: `cursor-status-fallback-${entry.id}`, type: 'status_block', entry })
  })
...
function processTone(kind: CursorProcessKind): { border: string; icon: string; gradient: string } {
  if (kind === 'command') {
    return {
...
function ProcessBlock({
  ...
  item: Extract<CursorTimelineItem, { type: 'process_block' }>
}): React.ReactNode {
...
function CliToolTimeline({ items }: { items: CursorTimelineItem[] }): React.ReactNode {
  if (items.length === 0) {
...
export function parseCursorAgentLogs(logs: LogMsg[]): NormalizedEntry[] {
...
export function CursorAgentLogView({ logs }: { logs: LogMsg[] }): React.ReactNode {
  ...
  const items = useMemo(() => buildCursorAgentTimelineItems(entries), [entries])
