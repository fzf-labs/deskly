...
type OpencodeProcessKind = 'command' | 'file_read' | 'file_edit' | 'tool'
...
function processKindFromEntry(entry: NormalizedEntry): OpencodeProcessKind {
  if (entry.type === 'command_run') return 'command'
...
function processTitle(kind: OpencodeProcessKind, entry: NormalizedEntry): string {
...
export function buildOpencodeTimelineItems(entries: NormalizedEntry[]): OpencodeTimelineItem[] {
  const items: OpencodeTimelineItem[] = []
  ...
  const flushAssistantBuffer = () => {
    ...
    if (content.trim()) {
      items.push({
        id: `opencode-answer-${assistantBuffer[0]?.id ?? items.length}`,
        type: 'answer_block',
  ...
  entries.forEach((entry, index) => {
    if (entry.type === 'user_message') {
      ...
      items.push({ id: `opencode-user-${entry.id}`, type: 'user_bubble', entry })
      return
    }
    ...
    if (entry.type === 'error') {
      items.push({ id: `opencode-error-${entry.id}`, type: 'error_block', entry })
      return
    }

    if (entry.type === 'system_message') {
      items.push({ id: `opencode-status-${entry.id}`, type: 'status_block', entry })
      return
    }
    ...
    items.push({ id: `opencode-status-fallback-${entry.id}`, type: 'status_block', entry })
  })
...
function processTone(kind: OpencodeProcessKind): { border: string; icon: string; gradient: string } {
  if (kind === 'command') {
    return {
...
function ProcessBlock({
  ...
  item: Extract<OpencodeTimelineItem, { type: 'process_block' }>
}): React.ReactNode {
...
function CliToolTimeline({ items }: { items: OpencodeTimelineItem[] }): React.ReactNode {
  if (items.length === 0) {
...
export function parseOpencodeLogs(logs: LogMsg[]): NormalizedEntry[] {
...
export function OpencodeLogView({ logs }: { logs: LogMsg[] }): React.ReactNode {
  ...
  const items = useMemo(() => buildOpencodeTimelineItems(entries), [entries])
