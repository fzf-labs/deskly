...
export const ToolSelectionContext = createContext<ToolSelectionContextType | null>(null)

export function useToolSelection() {
  const context = useContext(ToolSelectionContext)
  if (!context) {
    throw new Error('useToolSelection must be used within ToolSelectionContext')
  }
  return context
}
