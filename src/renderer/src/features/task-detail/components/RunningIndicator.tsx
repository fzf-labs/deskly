...
export function RunningIndicator({ messages }: RunningIndicatorProps) {
  const lastToolUse = [...messages].reverse().find((message) => message.type === 'tool_use')
  ...
  const getActivityText = () => {
    ...
    switch (lastToolUse.name) {
      ...
      case 'Read': {
        const readFile = input?.file_path ? String(input.file_path).split('/').pop() : ''
        return `Reading ${readFile || 'file'}...`
      }
      case 'Write': {
        const writeFile = input?.file_path ? String(input.file_path).split('/').pop() : ''
        return `Writing ${writeFile || 'file'}...`
      }
      case 'Edit': {
        const editFile = input?.file_path ? String(input.file_path).split('/').pop() : ''
