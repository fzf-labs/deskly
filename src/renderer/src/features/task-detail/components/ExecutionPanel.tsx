...
export function ExecutionPanel({
  ...
  return (
    <section
      className={cn(
        'border-border/50 bg-background flex min-h-0 flex-col overflow-hidden rounded-xl border shadow-sm',
        className
      )}
    >
      <div className="border-border/50 bg-background/95 sticky top-0 z-10 flex items-center justify-between gap-2 border-b px-3 py-2 backdrop-blur">
        <div className="text-muted-foreground flex items-center gap-2 text-xs font-semibold">
          ...
          <span className="text-foreground text-xs font-medium">{cliToolLabel}</span>
        </div>
        {cliStatusInfo && (
          <span className={cn('rounded-full px-2 py-0.5 text-xs font-medium', cliStatusInfo.color)}>
            {cliStatusInfo.label}
          </span>
        )}
      </div>
