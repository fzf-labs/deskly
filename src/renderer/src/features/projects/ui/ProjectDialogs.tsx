  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === 'clone') {
        ...
        const result = (await window.api.git.clone(remoteUrl, clonePath)) as {
          success: boolean
          error?: string
        }
        if (!result.success) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={cn(
              ...
              mode === 'local' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'
            )}
          >
            本地项目
          </button>
          <button
            ...
            className={cn(
              'flex-1 py-2 px-3 rounded-lg text-sm transition-colors',
              mode === 'clone' ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'
            )}
          >
            克隆仓库
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'clone' && (
            <div>
              <label className="text-sm font-medium mb-1.5 block">仓库地址</label>
              <input
                type="text"
                value={remoteUrl}
