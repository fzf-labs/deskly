...
export function AccountSettings({ settings, onSettingsChange }: SettingsTabProps) {
  const { t } = useLanguage()

  return (
    <div className="space-y-6">
      <div>
        <p className="text-muted-foreground text-sm">{t.settings.manageProfile}</p>
      </div>

      <div className="flex flex-col gap-2">
        <label className="text-foreground block text-sm font-medium">{t.settings.nickname}</label>
        <input
          type="text"
