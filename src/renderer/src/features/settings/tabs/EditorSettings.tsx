...
export function EditorSettings({ settings, onSettingsChange }: SettingsTabProps) {
  const { t } = useLanguage()
  ...
  const availableEditors = useMemo(() => editors.filter((editor) => editor.available), [editors])
    ...
    <div className="space-y-6">
      ...
      {selectedType === 'custom' && (
        <div className="flex flex-col gap-2">
          ...
          <input
            type="text"
            placeholder={t.settings?.editorCustomCommandPlaceholder || 'e.g., code'}
          ...
          />
          <p className="text-muted-foreground text-sm">
            {t.settings?.editorCustomCommandDesc || 'Enter the command used to launch your editor.'}
          </p>
        </div>
      )}
    </div>
