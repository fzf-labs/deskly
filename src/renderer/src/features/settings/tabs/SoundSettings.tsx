...
const RESOURCE_SOUND_VALUE_SET = new Set(RESOURCE_SOUND_OPTIONS.map((option) => option.value))

export function SoundSettings({ settings, onSettingsChange }: SettingsTabProps) {
  const { t } = useLanguage()
  ...
  const normalizeChoice = (choice: SoundChoice | undefined, fallbackFile: string): SoundChoice => {
  ...
  const ensureElectron = () => {
    if (typeof window === 'undefined' || !window.api?.dialog || !window.api?.fs) {
      throw new Error(t.settings?.soundFileUnavailable || 'File picker is not available.')
    }
  }
  ...
  const resolveResourceSelectValue = (choice: SoundChoice, fallbackFile: string): string => {
      ...
      <div className="border-border rounded-lg border p-4">
        <div className="space-y-3">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-1">
                <p className="text-sm font-medium">
                  {t.settings?.soundTaskCompleteLabel || 'Task sound'}
                </p>
          ...
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-xs">
              {t.settings?.soundCustomFileDesc || 'Use your own audio file for this alert.'}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <Button
        ...
        <div className="space-y-3">
          <div>
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-1">
                <p className="text-sm font-medium">
                  {t.settings?.soundTaskNodeCompleteLabel || 'Task node sound'}
                </p>
                <p className="text-muted-foreground text-xs">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {nodeIsCustomFile && (
              <Button
                ...
                disabled={
                  filePickerDisabled ||
                  !settings.taskNodeCompleteSoundEnabled ||
                  previewLocks.node
                }
              >
                {t.settings?.soundPreview || 'Preview'}
