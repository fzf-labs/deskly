...
export function NotificationSettings({ settings, onSettingsChange }: SettingsTabProps) {
  const { t } = useLanguage()
    ...
    <div className="space-y-6">
      <div className="border-border rounded-lg border p-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <Switch
            ...
            onChange={(enabled) => handleToggle('taskCompleteNotificationsEnabled', enabled)}
      ...
      <div className="border-border rounded-lg border p-4">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <p className="text-sm font-medium">
              {t.settings?.notificationTaskNodeTitle || 'Task node completion notifications'}
            </p>
            <p className="text-muted-foreground text-xs">
          ...
          <Switch
            ...
            onChange={(enabled) => handleToggle('taskNodeCompleteNotificationsEnabled', enabled)}
            disabled={!supported || isRequesting}
          />
        </div>
      </div>
