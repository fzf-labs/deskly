...
interface WorkflowTemplateEditorProps {
  ...
  preferredGenerationToolId?: string
  preferredGenerationAgentToolConfigId?: string
  nodeRuntimeDefaults?: {
...
const EDITOR_CANVAS_TOOLBAR_GROUP_CLASS =
  'flex items-center gap-0.5 rounded-[8px] border border-slate-200/70 bg-slate-50/90 p-0.5'
const EDITOR_CANVAS_TOOL_BUTTON_CLASS =
  'h-8 rounded-[6px] px-2.5 text-[11px] font-medium text-slate-500 hover:bg-white hover:text-slate-800'
...
export function WorkflowTemplateEditor({
  ...
  const autoGeneratedPromptRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const preferredToolId =
      preferredGenerationToolId === 'claude-code' || preferredGenerationToolId === 'codex'
        ? preferredGenerationToolId
        : ''
    const nextToolId =
      generationCliTools.find((tool) => tool.id === preferredToolId)?.id ||
      generationCliTools.find((tool) => tool.id === defaultCliToolId)?.id ||
      generationCliTools[0]?.id ||
      ''
  ...
  useEffect(() => {
    ...
    const syncGenerationConfig = async () => {
      ...
      const preferredConfigId =
        generationToolId === preferredGenerationToolId ? preferredGenerationAgentToolConfigId : ''
      const nextConfigId =
        configs.find((config) => config.id === preferredConfigId)?.id ||
  ...
  useEffect(() => {
    ...
    setError(null)
    setGenerationPrompt(initialGenerationPrompt || '')
    setGenerationToolId('')
    setGenerationAgentToolConfigId('')
    setSelectedEdgeId(null)
    autoGeneratedPromptRef.current = null
  ...
  }, [active, initialGenerationPrompt, initialValues])
  ...
  useEffect(() => {
    ...
    const settings = getSettings()
    const defaultCliToolId = getEnabledDefaultCliToolId(settings)
    const preferredCliToolId =
      nodeRuntimeDefaults?.cliToolId && cliTools.some((tool) => tool.id === nodeRuntimeDefaults.cliToolId)
        ? nodeRuntimeDefaults.cliToolId
        : ''
    const nextToolId =
      cliTools.find((tool) => tool.id === preferredCliToolId)?.id ||
      cliTools.find((tool) => tool.id === defaultCliToolId)?.id ||
      cliTools[0]?.id ||
      ''
  ...
  }, [active, cliTools, nodeRuntimeDefaults?.cliToolId, selectedNode, updateNode])
  ...
  useEffect(() => {
    ...
    const syncSelectedNodeConfig = async () => {
      ...
      const preferredConfigId =
        selectedNode.cliToolId === nodeRuntimeDefaults?.cliToolId
          ? nodeRuntimeDefaults?.agentToolConfigId
          : ''
      const nextConfigId =
        configs.find((config) => config.id === preferredConfigId)?.id ||
        configs.find((config) => config.is_default)?.id || configs[0]?.id || ''
  ...
  const focusTemplateNameField = useCallback(() => {
    ...
    window.setTimeout(() => {
      templateNameInputRef.current?.scrollIntoView({
        block: 'center',
        behavior: 'smooth'
      })
  ...
  const handleGenerate = useCallback(
    async (promptOverride?: string) => {
      const nextPrompt = (promptOverride ?? generationPrompt).trim()
      if (!nextPrompt) {
        setError(t.task.workflowGenerationPromptRequired || '请先输入工作流目标。')
        return
      }

      if (promptOverride !== undefined && promptOverride !== generationPrompt) {
        setGenerationPrompt(promptOverride)
      }
  ...
  useEffect(() => {
    const trimmedPrompt = initialGenerationPrompt?.trim()
    if (!active || !isTaskDraft || !trimmedPrompt || isGenerating) {
      return
    }

    if (autoGeneratedPromptRef.current === trimmedPrompt) {
      return
    }

    autoGeneratedPromptRef.current = trimmedPrompt
        ...
        <div className="space-y-3">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
              ...
              >
                {isTaskDraft ? t.task.generatedWorkflowReviewBadge : isDirty ? '未保存' : '已保存'}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2 xl:justify-end">
              <div className={EDITOR_TOOLBAR_GROUP_CLASS}>
                <EditorToolButton
                  icon={ChevronLeft}
                  label={isTaskDraft ? t.task.generatedWorkflowBackButton : t.common.cancel}
                  variant="ghost"
                ...
                <EditorToolButton
                  icon={Save}
                  label={
                    saving ? (savingLabel || t.task.createLoading) : (submitLabel || t.task.saveTemplate)
                  }
                  type="submit"
                  variant="default"
                  disabled={saving || secondaryActionLoading}
                      ...
                      <div className="space-y-4 px-4 py-4">
                        <div>
                          ...
                          <input
                            ...
                            aria-invalid={Boolean(templateNameError)}
                            aria-describedby={
                              templateNameError ? 'workflow-template-name-error' : undefined
                            }
                            className={cn(
                              EDITOR_INPUT_CLASS,
                              templateNameError &&
                                'border-red-300/80 bg-red-50/40 focus:border-red-400/80 focus:ring-red-500/15'
                            )}
                          />
                          {templateNameError ? (
                            <div
                              id="workflow-template-name-error"
          ...
          <div className={cn(EDITOR_PANEL_HEADER_CLASS, 'bg-white')}>
            <div className="flex w-full items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                <div className={EDITOR_CANVAS_TOOLBAR_GROUP_CLASS}>
                  <EditorToolButton
                    icon={GitBranchPlus}
                    label={t.task.addStage}
                    variant="ghost"
                    className={EDITOR_CANVAS_TOOL_BUTTON_CLASS}
                    onClick={addNode}
                  />
                  <EditorToolButton
                    icon={LayoutTemplate}
