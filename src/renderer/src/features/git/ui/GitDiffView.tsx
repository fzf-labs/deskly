...
function FileHeader({ file }: { file: ParsedDiffFile }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {file.isNew && (
        <span className="rounded bg-emerald-500/15 px-2 py-0.5 text-xs text-emerald-600">新增</span>
      )}
      {file.isDeleted && (
        <span className="rounded bg-rose-500/15 px-2 py-0.5 text-xs text-rose-600">删除</span>
      )}
      {file.isRenamed && (
        <span className="rounded bg-amber-500/15 px-2 py-0.5 text-xs text-amber-600">重命名</span>
      )}
      {!file.isNew && !file.isDeleted && !file.isRenamed && (
        <span className="rounded bg-muted px-2 py-0.5 text-xs text-muted-foreground">修改</span>
      )}
...
export function GitDiffView({
  ...
  if (error) {
    return (
      <div
        className={cn(
          'flex h-full flex-col items-center justify-center gap-2 text-center text-sm',
          className
        )}
      >
    ...
    <div className={cn('flex h-full flex-col gap-4 overflow-auto', className)}>
      <div className="flex items-center justify-between">
        <div className="text-xs text-muted-foreground">
          共 {groups.reduce((acc, group) => acc + group.files.length, 0)} 个文件
        </div>
