  ...
  if (diff.isBinary) {
    return (
      <div className={cn('rounded-lg border p-4', className)}>
        <div className="text-muted-foreground text-sm">Binary file not shown</div>
      </div>
    )
  }
  ...
  return (
    <>
      <tr className="bg-blue-500/10">
        <td colSpan={4} className="text-muted-foreground px-4 py-1 font-mono text-xs">
...
function DiffLineView({ line }: { line: DiffLine }) {
  const bgColor =
    line.type === 'add' ? 'bg-green-500/10' : line.type === 'delete' ? 'bg-red-500/10' : ''
  ...
  return (
    <tr className={bgColor}>
      <td className={cn('w-12 select-none px-2 py-0 text-right font-mono text-xs', lineNumColor)}>
        {line.oldLineNumber ?? ''}
      </td>
      ...
      <td className="w-6 select-none px-1 py-0 text-center">
        ...
        {line.type === 'delete' && <Minus className="inline size-3 text-red-600" />}
      </td>
