...
export function TerminalView({ paneId, cwd, workspaceId, isActive }: TerminalViewProps) {
  ...
  const startSession = useCallback(async (size?: { cols: number; rows: number }) => {
  ...
  useEffect(() => {
    ...
    const initFrame = window.requestAnimationFrame(() => {
      ...
      noOutputTimer = window.setTimeout(() => {
        ...
        xterm.writeln('[Try pressing Enter. If it is still blank, check the pnpm dev logs for TerminalService messages.]')
      }, 1500)
