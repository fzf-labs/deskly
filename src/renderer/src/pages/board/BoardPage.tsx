...
export function BoardPage() {
  ...
  const tasksByStatus = columns.reduce(
    (acc, col) => {
      acc[col.id] = tasks.filter((task) => normalizeTaskStatus(task.status) === col.id)
      return acc
    },
    ...
    if (currentProject?.path) {
      try {
        ...
        if (editorCommand && window.api?.editor?.openProject) {
          ...
          await window.api.editor.openProject(currentProject.path, editorCommand)
          return
        }
  ...
  return (
    <PageFrame>
      <PageHeader
        title="任务看板"
        subtitle={currentProject ? currentProject.name : '全部项目'}
        actions={
...
function BoardColumn({
  ...
  return (
    <div
      className={cn(
        'surface-card flex min-w-[240px] flex-1 flex-col rounded-[24px] border border-border/70 bg-card/85',
    ...
    >
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-2">
        {loading ? (
          ...
          <div className="text-muted-foreground p-4 text-center text-sm">暂无任务</div>
        ) : (
          tasks.map((task) => (
            <TaskCard
              key={task.id}
