...
* 第一版只支持“整张图重建”，不支持基于当前草稿继续扩展/重生成；但接口设计应为后续扩展预留空间。
* 第一版要明确模型生成时可用的上下文范围：
  * 第一版仅使用用户输入目标 + 当前模板名称
  * 不把当前已有节点草稿作为模型上下文输入
...
* [x] 明确第一版是否带规则回退方案
* [x] 明确生成输出的结构约束、校验、错误处理与用户可见行为
* [x] 明确生成范围是“全新草稿生成”还是也支持“基于当前图再生成”
...
* 保持现有 renderer 入口不变，继续通过 `db.generateWorkflowDefinition(...)` 发起生成。
* 将 main 层 `generateWorkflowDefinition` 从同步规则函数改为异步模型生成流程。
* 在 `WorkflowDefinitionGenerationService` 中新增一条基于现有 CLI runtime 的短生命周期生成路径：
  * 组装 workflow generation 专用 prompt
  * 注入结构化输出 schema
  * 启动内部 CLI 生成会话
  * 读取最终 JSON 结果
* 模型返回结果后，统一执行：
  * JSON parse
  * 结构规范化
  * `WorkflowDefinitionService` DAG 校验
  * 成功后回传 renderer
* 若模型调用失败、超时、解析失败或校验失败：
  * renderer 显示明确错误
  * 提示用户是否回退到当前规则生成
  * 仅在用户确认时执行回退
* 第一版只支持“整张图重建”：
  * 输入上下文为“目标 + 模板名称”
  * 不读取当前编辑中的 nodes/edges 作为模型上下文
  * 但类型和 service 接口命名要为未来“基于当前图扩展”预留空间
