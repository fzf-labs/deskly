...
* 底层 CLI 选择
* 默认 runtime profile（通过引用现有 `AgentToolConfig`）
* skills 显式绑定方式
* role / task 指令注入方式
* conversation / workflow 中的一致执行方式
* `AgentToolConfig.tool_id` 是运行时 CLI 的唯一真相来源；第一版 Agent 不再额外持久化一份可能冲突的 `cliToolId`。
* 第一版中，Agent 的 skill 绑定必须是“可执行语义”，而不是纯元数据提示：
* 至少需要定义为运行时 allowlist / selected skills 集合。
* run snapshot 不能只记录 skill 名称或路径；需要固化 skill revision、hash 或内容快照，避免历史运行随 skill 文件变化而漂移。
* 第一版中，conversation 场景下的 Agent 身份必须依赖会话级配置持续生效，而不能只依赖首条拼接 prompt：
* 稳定角色约束应尽量落在 runtime profile / system prompt / session-level 配置。
* task prompt 只负责当前实例需求，不承担长期角色维持职责。
* 第一版中，prompt 组合顺序固定为：
* `Agent prompt`
* `workflow node step prompt`（如果是 workflow）
* `task prompt`
* 这样 Agent 负责稳定角色定义，node 负责步骤职责，task 负责当前实例需求。
* 需要明确 Agent 在 Deskly 里的主要进入点和复用路径。
* 第一版 Agent 同时覆盖 `conversation task` 与 `workflow node` 两个入口，保证概念统一。
* 需要明确 Agent 如何在现有 conversation task 与 workflow node 中使用。
* 需要支持 `global + project` 两种 Agent 作用域，并定义选择、展示与复用规则。
* 第一版 Agent 作为新增高层入口，与现有手动 `CLI/profile/prompt` 配置模式并存，不直接替代旧入口。
* 第一版中，workflow node / conversation task 在引用 Agent 后不覆盖 Agent 默认配置，保证 Agent 语义稳定。
* 第一版中，conversation 模板 / workflow 模板引用 `agentDefinitionId`，未来运行默认解析最新的已发布 Agent；真正的冻结发生在 run snapshot 创建时。
* 第一版需要最小化发布边界，避免 “编辑 Agent = 隐形影响所有未来运行”：
* 至少需要引入 `draft/published` 或等价 revision guard。
* 模板默认解析最新已发布 revision；run snapshot 记录具体 agent revision。
* 需要明确 workflow run 如何固化 agent 的运行时快照，避免后续配置漂移影响历史记录。
* 需要定义第一版的引用约束矩阵，避免编辑态和运行态出现双重真相：
* `global workflow` 只能引用 `global agent`。
* `project workflow` 可以引用 `global agent` 或同项目 `project agent`。
* `conversation task` 在项目上下文内可引用 `global/project agent`；无项目上下文时只能引用 `global agent`。
* 编辑态中，节点若设置 `agentDefinitionId`，则不再允许同时手动编辑 Agent 管辖的 legacy 字段。
* run snapshot 展开后，运行节点仍落到现有 `cli_tool_id + agent_tool_config_id + prompt + skills snapshot` 形态。
* `automation / scheduler` 不在第一版范围内，后续在 Agent 模型稳定后再接入。
...
* [ ] 能明确第一版 workflow node 保留自己的步骤级 prompt。
* [ ] 能明确第一版模板引用最新 Agent，而运行记录通过 snapshot 保证可复现。
* [ ] 能明确第一版 Agent 通过引用现有 `AgentToolConfig` 复用 runtime profile，而不是内嵌重复配置。
* [ ] 能明确第一版 Agent 同时集成到 `conversation task` 与 `workflow node`。
* [ ] 能明确第一版 Agent 与现有手动 `CLI/profile/prompt` 模式并存，而不是直接替代旧入口。
* [ ] 能明确第一版不接 `automation / scheduler`。
* [ ] 能明确 `AgentToolConfig.tool_id` 是运行时 CLI 的唯一真相来源。
* [ ] 能明确 skill 绑定在第一版具有可执行语义，并在 snapshot 中防止 skill 文件漂移。
* [ ] 能明确 conversation 场景下 Agent 身份依赖会话级配置持续生效，而不只是首条 prompt。
* [ ] 能明确模板解析“最新已发布 Agent”，而不是任意草稿变更。
* [ ] 能明确 `global/project` scope 的引用约束矩阵与 legacy 字段互斥规则。
...
* 第一版不接入 `automation / scheduler` 直接运行 Agent。
* 第一版不做完整的 Agent 历史版本管理、diff 浏览或复杂升级迁移界面；只做满足安全发布的最小 revision / published 机制。
...
* 当前 `WorkflowRunService` 创建运行节点时，只会把 `task.prompt` 与 `node.prompt` 组合进最终 prompt，然后将 `cli_tool_id` 与 `agent_tool_config_id` 落库。
* 当前 `CliSessionService.startSession` 只在 session 启动时拼接一次 prompt；后续 `sendInput` 是原样发送用户输入。
* 这意味着：
* 如果 Agent 身份只存在于首轮拼接文本中，conversation 后续轮次无法可靠维持相同角色边界。
* 如果要做到“显式 skill 绑定 + 可复现”，run snapshot 必须比今天的 prompt 拼接模型携带更多结构化信息。
...
* Cons:
* 需要新增一层模型和解析逻辑。
* 要解决 snapshot 固化、技能绑定、发布边界和 UI 管理边界。
...
* Agent 若引用已删除或跨 scope 非法的 runtime profile，需要在模板保存和运行创建时双重校验。
* `latest published` 模式下，Agent 的草稿编辑不能悄悄影响现有模板的未来运行。
* global/project 混用时，项目外上下文不能看到或解析项目私有 Agent。
...
* Agent 第一版将引用现有 `AgentToolConfig` 作为 runtime profile，而不是自带一份独立的 CLI 配置副本；`AgentToolConfig.tool_id` 是运行时 CLI 的唯一真相来源。
* Agent 第一版将显式绑定 skill 列表，而不是只做 prompt 层面的推荐说明。
* skill 绑定在第一版必须具有可执行语义，且 run snapshot 需要固化 skill revision/hash/内容快照以保证可复现。
* Agent 第一版支持 `global + project` 两种作用域，思路与现有 workflow definition scope 保持一致。
* Agent 第一版同时集成到 `conversation task` 与 `workflow node`，避免做成只属于某一条链路的特例。
* 现有手动 `CLI/profile/prompt` 配置入口保留，Agent 作为新增高层入口渐进接入，降低迁移风险。
* workflow 节点 / conversation task 在第一版不覆盖 Agent 默认值，因此 Agent 是强语义能力单元，而不是可随意改写的初始化模板。
* prompt 组合规则固定为“Agent prompt 在前，step prompt 居中，task prompt 在后”，但 conversation 中稳定角色定义必须主要落到会话级配置而非仅首轮文本。
* conversation 模板 / workflow 模板 默认引用最新的已发布 Agent；历史运行通过 snapshot 固化解析后的配置，避免配置漂移。
* 第一版引入最小 revision / published 机制，避免草稿编辑隐式影响所有未来运行。
* workflow 节点后续可能需要新增 `agentDefinitionId`，并在 run snapshot 时解析为具体的 `agent revision + cli_tool_id + agent_tool_config_id + prompt + skills snapshot`。
* 需要定义 scope 约束矩阵和 legacy 字段互斥规则，避免编辑态与运行态双重真相。
* `automation / scheduler` 暂不接入，避免第一版同时改动编排、自动化和资产管理三条链路。
...
* 当前 repo 已有成熟的 `AgentToolConfig` CRUD 和解析链路，因此第一版 Agent 复用它比内嵌一套重复 runtime config 更合适。
* 逻辑补强后，第一版 MVP 还需要补上的最小契约是：
* skill 绑定的可执行语义
* conversation 会话级 Agent 身份
* `latest published` 的最小发布边界
* `tool_id` 单一真相来源
* scope / legacy 互斥矩阵
* 补充用户使用与操作流程图文档：
